...
class NGINXConfigWrapper(object):
    ...
    def load(self, filename=None, reload=False):
        ...
        try:
            ...
            self.config = nginxparser.load(self.file)
        except Exception as err:
    ...
    def write_config(self):
        try:
            ...
            module.fail_json(msg="Failed to write NGINX configuration file %s: %s"
                             % (filename, str(err)))
    ...
    def save_config(self):
        ...
        if not self.sabconfig.save_config():
            self.module.fail_json(msg="Can't save NGINX config file %s" %
                                  self.filename)
...
def main():
    ...
    import pprint
    module.exit_json(msg=pprint.pformat(sys.path))
